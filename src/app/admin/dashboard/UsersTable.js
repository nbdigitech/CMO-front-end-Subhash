...
export default function UsersTable() {
  ...
  const toggleStatus = async (id, currentStatus) => {
    try {
      const response = await fetch(`https://api.cmophotos.in//update-user/${id}`, {
        method: "PUT",
  ...
  const fetchUsers = async () => {
    try {
      const response = await fetch("https://api.cmophotos.in//users");
  ...
  if (password && password.length > 0) {
    ...
    try {
      const response = await fetch(`https://api.cmophotos.in//update-user/${editedUser._id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("https://api.cmophotos.in//add-staff", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
