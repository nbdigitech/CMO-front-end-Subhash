...
const AllPhotos = ({ currentTab } ) => {
  ...
  const fetchAlbums = async () => {
    try {
      const response = await fetch("https://api.cmophotos.in//albums");
      if (!response.ok) {
        throw new Error("Failed to fetch albums");
      }
      const data = await response.json();
  ...
  if (!album || !album._id) return;
  try {
    const response = await fetch(`https://api.cmophotos.in//photos/${album._id}`);
    if (!response.ok) throw new Error("Failed to fetch photos");
    const data = await response.json();
    console.log("📸 Fetched Photos:", data);
  ...
  const handleDownload = async () => {
    ...
    for (const item of selectedItems) {
        if (item.photo_id) {
            ...
            // ✅ Item is an album
            try {
                const response = await fetch(`https://api.cmophotos.in//photos/${item._id}`);
                if (!response.ok) throw new Error("Failed to fetch photos");
                const data = await response.json();
        ...
        try {
          if (selectedAlbum) {
            ...
            for (const photo of selectedItems) {
              ...
              const response = await fetch(
                `https://api.cmophotos.in//photo/${selectedAlbum._id}/${photo.photo_id}`,
                { method: "DELETE" }
              );
            ...
            // Deleting albums
            const response = await fetch("https://api.cmophotos.in//delete-albums", {
              method: "DELETE",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ albumIds: selectedItems.map((album) => album._id) }),
            });
